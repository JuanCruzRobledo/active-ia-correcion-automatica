...
Migrar la fuente de credenciales Moodle (host, usuario, contraseña) de campos globales por usuario (`usuarios.moodle_*`) a membresía por universidad (`UsuarioUniversidad.moodle_*`), habilitando multi-tenancy en los flujos Moodle y garantizando un fail-fast (HTTP 424) cuando el campus de la universidad activa no esté configurado.
